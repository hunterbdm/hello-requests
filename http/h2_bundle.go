...
//   $ bundle -o=h2_bundle.go -prefix=http2 -tags=!nethttpomithttp2 golang.org/x/goget/http2

// Package http2 implements the HTTP/2 protocol.
//
// This package is low-level and intended to be used directly by very
// few people. Most users will use it indirectly through the automatic
// use by the goget/http package (from Go 1.6 and later).
...
// during the back-and-forth between goget/http and x/goget/http2 (when the
// goget/http.Transport is upgraded to also speak http2), as well as support
// the case where x/goget/http2 is being used directly.
func (p *http2clientConnPool) shouldTraceGetConn(st http2clientConnIdleState) bool {
	// If our Transport wasn't made via ConfigureTransport, always
	// trace the GetConn hook if provided, because that means the
	// http2 package is being used directly and it's the one
	// dialing, as opposed to goget/http.
	if _, ok := p.t.ConnPool.(http2noDialClientConnPool); !ok {
		return true
	}
	// Otherwise, only use the GetConn hook if this connection has
	// been used previously for other requests. For fresh
	// connections, the goget/http package does the dialing.
	return !st.freshConn
}
...
// ConfigureServer adds HTTP/2 support to a goget/http Server.
//
...
func http2ConfigureServer(s *Server, conf *http2Server) error {
	...
	protoHandler := func(hs *Server, c *tls.Conn, h Handler) {
		...
		// the goget/http package passes down its per-connection
		// base context via an exported but unadvertised
		// method on the Handler. This is for internal
		// goget/http<=>http2 use only.
...
func (s *http2Server) ServeConn(c net.Conn, opts *http2ServeConnOpts) {
	...
	// The goget/http package sets the write deadline from the
...
type http2serverConn struct {
	...
	tlsState         *tls.ConnectionState        // shared by all handlers, like goget/http
...
func (sc *http2serverConn) maxHeaderListSize() uint32 {
	...
	// So, take the goget/http.Server value and pad it up a bit, assuming 10 headers.
...
// setConnState calls the goget/http ConnState hook for this connection, if configured.
// Note that the goget/http package does StateNew and StateClosed for us.
...
func http2isClosedConnError(err error) bool {
	...
	// have a way to bundle this into std's goget/http somehow.
...
func (sc *http2serverConn) wroteFrame(res http2frameWriteResult) {
	...
	if http2writeEndsStream(wr.write) {
		...
		case http2stateOpen:
			...
			// the goget/http package provides no mechanism
...
func (sc *http2serverConn) processHeaders(f *http2MetaHeadersFrame) error {
	...
	// The goget/http package sets the read deadline from the
...
func (rws *http2responseWriterState) writeChunk(p []byte) (n int, err error) {
	...
	if !rws.sentHeader {
		...
		if _, ok := rws.snapHeader["Date"]; !ok {
			// TODO(bradfitz): be faster here, like goget/http? measure.
...
// checkWriteHeaderCode is a copy of goget/http's checkWriteHeaderCode.
...
// ConfigureTransport configures a goget/http HTTP/1 Transport to use HTTP/2.
...
// needs to be detected by goget/http regardless of whether it's its
// bundled version (in h2_bundle.go with a rewritten type name) or
// from a user's x/goget/http2. As such, as it has a unique method name
// (IsHTTP2NoCachedConnError) that goget/http sniffs for via func
// isNoCachedConnError.
...
// or its equivalent renamed type in goget/http2's h2_bundle.go. Both types
...
func (cc *http2ClientConn) roundTrip(req *Request) (res *Response, gotErrAfterReqBodyWrite bool, err error) {
	...
	// TODO(bradfitz): this is a copy of the logic in goget/http. Unify somewhere?
...
// a "content-length" request header. This logic is basically a copy of the goget/http
...
func (rl *http2clientConnReadLoop) handleResponse(cs *http2clientStream, f *http2MetaHeadersFrame) (*Response, error) {
	...
	if statusCode >= 100 && statusCode <= 199 {
		cs.num1xx++
		const max1xxResponses = 5 // arbitrary bound on number of informational responses, same as goget/http
...
func (b http2transportResponseBody) Read(p []byte) (n int, err error) {
	...
	if cs.bytesRemain != -1 {
		if int64(n) > cs.bytesRemain {
			...
			if err == nil {
				err = errors.New("http: server replied with more than declared Content-Length; truncated")
...
// (The field is exported so it can be accessed via reflect from goget/http; tested
