...
type http2Framer struct {
	...
	// [hello-requests] added this so the H2Settings can be used in the decoder
	t *http2Transport
}
...
func (f *http2Framer) logWrite() {
	if f.debugFramer == nil {
		...
		f.debugFramer = http2NewFramer(nil, f.debugFramerBuf, f.t)
...
func http2NewFramer(w io.Writer, r io.Reader, t *http2Transport) *http2Framer {
	fr := &http2Framer{
		w:                 w,
		r:                 r,
...
func (fr *http2Framer) readMetaFrame(hf *http2HeadersFrame) (*http2MetaHeadersFrame, error) {
	...
	// [hello-requests] fix COMPRESSION_ERROR here by setting the higher dynamic table size limit on the decoder
	if fr.t != nil && fr.t.MimicSettings != nil && len(fr.t.MimicSettings.H2Settings) > 0 {
		for _, setting := range fr.t.MimicSettings.H2Settings {
			if http2SettingID(setting.ID) == http2SettingHeaderTableSize {
				hdec.SetAllowedMaxDynamicTableSize(setting.Val)
				break
			}
		}
	}
...
func http2configureTransport(t1 *Transport) (*http2Transport, error) {
	...
	t2 := &http2Transport{
		...
		t1:       t1,
		MimicSettings: t1.MimicSettings,
	}
	connPool.t = t2
...
func (t *http2Transport) newClientConn(c net.Conn, singleUse bool) (*http2ClientConn, error) {
	...
	cc.fr = http2NewFramer(cc.bw, cc.br, t)
	...
	// [hello-requests] Set custom h2 settings here
	var initialSettings []http2Setting

	if t.MimicSettings != nil && len(t.MimicSettings.H2Settings) > 0 {
		for _, setting := range t.MimicSettings.H2Settings {
		...
		defaultConnFlow := uint32(http2transportDefaultConnFlow)
		if t.MimicSettings.H2StreamFlow != 0 {
			defaultConnFlow = t.MimicSettings.H2StreamFlow
		}

		cc.bw.Write(http2clientPreface)
		cc.fr.WriteSettings(initialSettings...)
		cc.fr.WriteWindowUpdate(0, defaultConnFlow)
		cc.inflow.add(int32(defaultConnFlow + http2initialWindowSize))

		for _, priorityFrame := range t.MimicSettings.H2PriorityFrames {
			cc.fr.WritePriority(priorityFrame.StreamID, http2PriorityParam{
				StreamDep: priorityFrame.StreamDep,
				Exclusive: priorityFrame.Exclusive,
				Weight: priorityFrame.Weight - 1,
			})
		}
	} else {
...
func (cc *http2ClientConn) encodeHeaders(req *Request, addGzipHeader bool, trailers string, contentLength int64) ([]byte, error) {
	...
	// [hello-requests] Added logic to handle custom HeaderOrder and H2HeaderOrder
	enumerateHeaders := func(f func(name, value string)) {
		...
		// [hello-requests] Write h2 only headers in specific order if provided
		if cc.t.MimicSettings != nil && len(cc.t.MimicSettings.H2HeaderOrder) == 4 {
			for _, headerName := range cc.t.MimicSettings.H2HeaderOrder {
				switch headerName {
				case ":authority":
					f(":authority", host)
					break
		...
		handleHeader := func(k string, vv []string) {
			if strings.EqualFold(k, "host") {
				...
				// Verify if we should add content-length header before doing so

				didContentLength = true
				if http2shouldSendReqContentLength(req.Method, contentLength) {
					f("content-length", vv[0])
				}
		...
		for _, headerName := range req.HeaderOrder {
			if strings.EqualFold(headerName, "content-length") {
				// Also use the calculated contentLength and not user provided
				// contentLength because the user input may be wrong
		...
		for k, vv := range req.Header {
			// Check if header was already written above
			var skipHeader bool
			for _, headerName := range req.HeaderOrder {
		...
		if !didContentLength {
			handleHeader("content-length", []string{strconv.FormatInt(contentLength, 10)})
		}
		if addGzipHeader {
