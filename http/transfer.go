...
// [hello-requests] This copies the logic from writeHeader but
// instead just sets the Header value so we can write it in the
// correct order later
func (t *transferWriter) addHeaders(h *Header) error {
	if t.Close && !hasToken(t.Header.get("Connection"), "close") {
		h.Set("Connection", "close")
	}

	// Add Content-Length and/or Transfer-Encoding whose values are a
	// function of the sanitized field triple (Body, ContentLength,
	// TransferEncoding)
	if t.shouldSendContentLength() {
